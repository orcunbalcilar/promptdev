...
vi.mock('@/components/kanban-board', () => ({
  KanbanBoard: ({ tasks, onTaskClick }: { tasks: { id: string; title: string }[]; onTaskClick: (t: { id: string; title: string }) => void }) => (
    <div data-testid="kanban-board" data-task-count={tasks.length}>
      {tasks.map((t) => (
        <button key={t.id} onClick={() => onTaskClick(t)} data-testid={`task-${t.id}`}>
          {t.title}
        </button>
      ))}
    </div>
...
vi.mock('@/components/ui/select', () => {
  ...
  return {
    Select: ({ value, onValueChange, children }: { value: string; onValueChange: (v: string) => void; children: React.ReactNode }) => {
      ...
      return (
        <select aria-label={id} data-testid={id} value={value} onChange={(e) => onValueChange(e.target.value)}>
          {children}
        </select>
      )
    },
...
const mockTasks = [
  { id: '1', title: 'Frontend Fix', prompt: 'Fix the login button', status: 'PENDING', createdAt: new Date().toISOString(), workspaceType: 'BITBUCKET' },
  { id: '2', title: 'Backend API', prompt: 'Add user endpoint', status: 'IN_PROGRESS', createdAt: new Date().toISOString(), workspaceType: 'LOCAL' },
  { id: '3', title: 'Review Code', prompt: 'Review PR 123', status: 'COMPLETED', createdAt: new Date().toISOString(), workspaceType: 'BITBUCKET' },
]

const successResponse = { content: mockTasks, totalElements: 3, totalPages: 1, number: 0, size: 100 }
const emptyResponse = { content: [], totalElements: 0, totalPages: 0, number: 0, size: 100 }
...
beforeEach(() => {
  ...
  globalThis.EventSource = vi.fn().mockImplementation(function (this: Record<string, unknown>, url: string) {
...
describe('Dashboard', () => {
  ...
  it('renders KanbanBoard when tasks are loaded', async () => {
    ...
    expect(screen.getByTestId('kanban-board')).toHaveAttribute('data-task-count', '3')
  ...
  it('filters tasks by search query (title match)', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('kanban-board')).toHaveAttribute('data-task-count', '1')
    })
    expect(screen.getByText('Frontend Fix')).toBeInTheDocument()
  ...
  it('creates SSE connection to /stream/tasks', async () => {
    ...
    await waitFor(() => {
      expect(mockEventSourceConstructor).toHaveBeenCalledWith('http://localhost:8080/api/stream/tasks')
    })
  })
})
