...
import { AgentActivityStream, ChangedFilesTree } from '@/components/agent-activity-stream'
import { TaskChangesSummary } from '@/components/task-changes-summary'
import { getMonitoringSessionDetails, type MonitoringSession } from '@/lib/monitoring'
import { ProgressBar, ProgressBarLabel, ProgressBarValue, ProgressBarTrack, ProgressBarFill } from '@/components/ai-elements/progress-bar'
...
function getStatusIndicatorStatus(taskStatus: string): 'streaming' | 'submitted' | 'error' | 'complete' {
  if (['PENDING', 'QUEUED'].includes(taskStatus)) return 'submitted'
  if (['IN_PROGRESS', 'TRIAGING', 'REVIEWING', 'COMMITTING', 'PUSHING', 'CREATING_PR', 'VALIDATING', 'ITERATION_PENDING', 'CODE_GENERATED'].includes(taskStatus)) return 'streaming'
      ...
      <CardContent className="px-4 pb-4">
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div className="space-y-0.5">
            ...
            <p className="font-mono font-semibold text-sm">{formatTokenCount(session.totalInputTokens)}</p>
          </div>
          <div className="space-y-0.5">
            <span className="text-muted-foreground font-medium">Output Tokens</span>
            <p className="font-mono font-semibold text-sm">{formatTokenCount(session.totalOutputTokens)}</p>
          </div>
          <div className="space-y-0.5">
            <span className="text-muted-foreground font-medium">Messages</span>
            <p className="font-mono font-semibold text-sm">{session.messageCount}</p>
          </div>
          <div className="space-y-0.5">
            <span className="text-muted-foreground font-medium">Tool Calls</span>
            <p className="font-mono font-semibold text-sm">{session.toolExecutionCount}</p>
          </div>
          {session.errorCount > 0 && (
            <div className="space-y-0.5 col-span-2">
              <span className="text-muted-foreground font-medium">Errors</span>
              <p className="font-mono font-semibold text-sm text-destructive">{session.errorCount}</p>
            </div>
          )}
        </div>
      </CardContent>
...
export default function TaskDetailPage() {
  ...
  const isLive = !['COMPLETED', 'FAILED', 'CANCELLED'].includes(task?.status ?? '')

  const handleCancel = async () => {
    if (!task) return
    if (!globalThis.confirm('Are you sure you want to cancel this task?')) return
      ...
      <main className="flex-1 overflow-hidden">
        <div className="flex h-[calc(100vh-4rem)] divide-x">
          ...
          <div className="w-80 shrink-0 overflow-y-auto p-4 space-y-4">
            ...
            <ProgressBar value={getTaskProgress(task.status)}>
              <ProgressBarLabel>
                ...
                <ProgressBarValue>{getTaskProgress(task.status)}%</ProgressBarValue>
              </ProgressBarLabel>
              <ProgressBarTrack>
            ...
            {/* Agent Status */}
            <div className="flex items-center gap-2">
              <StatusIndicator status={getStatusIndicatorStatus(task.status)} showIcon />
            </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm px-4 pb-4">
                ...
                <div className="space-y-1">
                  <span className="text-muted-foreground font-medium text-xs">Workspace</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <span className="text-muted-foreground font-medium text-xs">Source</span>
                  ...
                  <div className="space-y-1">
                    <span className="text-muted-foreground font-medium text-xs">Target</span>
                    <div className="font-mono text-xs flex items-center gap-1">
                      <GitBranch className="h-3 w-3" />
                      {task.targetBranch}
                    </div>
                ...
                {task.jiraIssueKey && (
                  <div className="space-y-1">
                    <span className="text-muted-foreground font-medium text-xs">Jira Issue</span>
                    <div className="flex items-center gap-2 text-xs">
                      <Bug className="h-3 w-3" />
                      <Badge variant="outline" className="font-mono text-[10px]">{task.jiraIssueKey}</Badge>
                    </div>
                  </div>
                )}
                ...
                {task.reviewEnabled !== undefined && (
                  <div className="space-y-1">
                    <span className="text-muted-foreground font-medium text-xs">Review</span>
                    <div className="flex items-center gap-2 text-xs">
                      <Shield className="h-3 w-3" />
                      <span>{task.reviewEnabled ? 'Enabled' : 'Disabled'}</span>
                      {task.reviewModelId && (
                        <Badge variant="secondary" className="text-[10px] ml-1">{task.reviewModelId}</Badge>
                      )}
                    </div>
                  </div>
                )}
                ...
                {task.skills && (
                  <div className="space-y-1">
                    <span className="text-muted-foreground font-medium text-xs">Skills</span>
                    <div className="flex flex-wrap gap-1">
                      {task.skills.split(',').map((s) => (
                        <Badge key={s.trim()} variant="outline" className="text-[10px] flex items-center gap-1">
                ...
                {(task.resumeCount ?? 0) > 0 && (
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-muted-foreground">Resumed {task.resumeCount} time{task.resumeCount === 1 ? '' : 's'}</span>
                  </div>
                )}

                <div className="grid grid-cols-2 gap-3 pt-2 border-t">
                  <div className="space-y-0.5">
                    <span className="text-muted-foreground font-medium text-[10px]">Created</span>
                  ...
                  {task.completedAt && (
                    <div className="space-y-0.5">
                      <span className="text-muted-foreground font-medium text-[10px]">Completed</span>
                      <div className="text-[11px] flex items-center gap-1">
            ...
            {/* Session Metrics */}
            {sessionDetails && (
              <SessionMetricsCard session={sessionDetails} />
            )}
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0">
            <Tabs defaultValue="activity" className="flex flex-col flex-1 min-h-0 gap-0">
              <div className="flex items-center justify-between px-4 py-2 border-b bg-muted/30">
                <TabsList variant="line" className="h-auto">
                  <TabsTrigger value="activity" className="text-xs gap-1.5 px-3 py-1.5">
                    <Bot className="h-3.5 w-3.5" />
                    Live Activity
                  ...
                  <TabsTrigger value="changes" className="text-xs gap-1.5 px-3 py-1.5">
                ...
                >
                  {showFilesPanel ? <PanelRightClose className="h-4 w-4" /> : <PanelRightOpen className="h-4 w-4" />}
                </Button>
              </div>
