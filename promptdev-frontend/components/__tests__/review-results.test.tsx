...
import type { ReviewResult } from '@/components/review-results'
...
function createResult(overrides: Partial<ReviewResult> & { severity: ReviewResult['severity'] }): ReviewResult {
  return {
...
describe('ReviewResults', () => {
  ...
  it('renders error findings with correct severity styling', () => {
    const results = [createResult({ severity: 'error', description: 'Missing null check' })]
  ...
  it('renders warning findings', () => {
    const results = [createResult({ severity: 'warning', description: 'Unused variable' })]
    render(<ReviewResults results={results} />)
  ...
  it('renders info findings', () => {
    const results = [createResult({ severity: 'info', description: 'Consider using const' })]
    render(<ReviewResults results={results} />)
  ...
  it('renders suggestion text when present', () => {
    ...
    render(<ReviewResults results={results} />)
    expect(screen.getByText('Use optional chaining instead')).toBeInTheDocument()
  })
...
describe('parseReviewResults', () => {
  ...
  it('parses nested { findings: [...] } format', () => {
    const input = { findings: [{ severity: 'warning', filePath: 'b.ts', description: 'warn' }] }
    const result = parseReviewResults(JSON.stringify(input))
    expect(result).toHaveLength(1)
  ...
  it('parses nested { results: [...] } format', () => {
    const input = { results: [{ severity: 'info', filePath: 'c.ts', description: 'note' }] }
    const result = parseReviewResults(JSON.stringify(input))
    expect(result).toHaveLength(1)
  ...
  it('parses nested { issues: [...] } format', () => {
    const input = { issues: [{ severity: 'error', filePath: 'd.ts', description: 'issue' }] }
    const result = parseReviewResults(JSON.stringify(input))
    expect(result).toHaveLength(1)
