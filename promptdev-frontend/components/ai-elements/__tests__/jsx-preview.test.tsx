...
vi.mock('react-jsx-parser', () => ({
  default: ({ jsx }: { jsx?: string; onError?: (err: Error) => void }) => <div data-testid="jsx-parser">{jsx}</div>,
}))
...
describe('JSXPreviewContent', () => {
  it('renders JSX string', () => {
    ...
    expect(screen.getByTestId('jsx-parser')).toHaveTextContent('<div>Hello World</div>')
  })
  ...
  it('renders complete JSX without modification when not streaming', () => {
    ...
    expect(screen.getByTestId('jsx-parser')).toHaveTextContent('<p>Complete</p>')
  })
})
