...
vi.mock('@/lib/monitoring', async (importOriginal) => {
  ...
  return {
    ...actual,
    getSessionOperations: (...args: unknown[]) => mockGetSessionOperations(...args),
  }
})
...
vi.mock('@/components/ai-elements/tool', () => ({
  ...
  ToolHeader: ({ title, state }: { title: string; state: string }) => (
    <div data-testid="tool-header" data-state={state}>{title}</div>
  ),
...
vi.mock('@/components/ai-elements/stack-trace', () => ({
  ...
  StackTraceHeader: ({ children }: { children?: ReactNode }) => <div>{children}</div>,
  StackTraceError: ({ children }: { children?: ReactNode }) => <div>{children}</div>,
  StackTraceErrorType: () => <span data-testid="error-type" />,
  StackTraceErrorMessage: () => <span data-testid="error-message" />,
  StackTraceContent: ({ children }: { children?: ReactNode }) => <div>{children}</div>,
...
vi.mock('@/components/ai-elements/progress-bar', () => ({
  ProgressBar: ({ children }: { children?: ReactNode }) => <div data-testid="progress-bar">{children}</div>,
...
function makeOperation(overrides: Partial<MonitoringOperation> = {}): MonitoringOperation {
  return {
...
describe('SessionDetail – extended operation rendering', () => {
  ...
  it('shows 0% progress for ACTIVE session with no operations', () => {
    renderWithProviders(
      <SessionDetail session={makeSession({ status: 'ACTIVE' })} onBack={vi.fn()} />,
    )
  ...
  it('shows 100% progress for ERROR session regardless of operations', () => {
    renderWithProviders(
      <SessionDetail session={makeSession({ status: 'ERROR' })} onBack={vi.fn()} />,
    )
  ...
  it('shows complete status indicator for ENDED session', () => {
    renderWithProviders(
      <SessionDetail session={makeSession({ status: 'ENDED' })} onBack={vi.fn()} />,
    )
    expect(screen.getByTestId('status-indicator')).toHaveAttribute('data-status', 'complete')
  })
  ...
  it('formats token counts (thousands)', () => {
    renderWithProviders(
      <SessionDetail
        session={makeSession({ totalInputTokens: 2000, totalOutputTokens: 500 })}
        onBack={vi.fn()}
      />,
    )
  ...
  it('renders tool operation with message as ToolInput', async () => {
    ...
    expect(await screen.findByTestId('tool-input')).toHaveTextContent('executing command')
  })
  ...
  it('uses fallback STATUS_CONFIG for unknown status', () => {
    const session = makeSession({ status: 'UNKNOWN' as MonitoringSession['status'] })
