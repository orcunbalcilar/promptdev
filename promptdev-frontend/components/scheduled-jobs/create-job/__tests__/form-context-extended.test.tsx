...
function FormConsumer({ onContext }: { onContext: (ctx: ReturnType<typeof useJobForm>) => void }) {
  ...
  const result = render(
    <QueryClientProvider client={createQueryClient()}>
      <JobFormProvider open={open} onClose={onClose}>
        <FormConsumer onContext={(ctx) => { contextRef = ctx }} />
      </JobFormProvider>
    </QueryClientProvider>,
  )
...
describe('JobFormProvider – resetForm', () => {
  it('resets all fields to defaults', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('workspace-type')).toHaveTextContent('BITBUCKET')
