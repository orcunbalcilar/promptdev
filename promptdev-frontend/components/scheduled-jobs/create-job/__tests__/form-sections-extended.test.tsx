...
vi.mock('@/components/shared/branch-selector', () => ({
  BranchSelector: (props: { selectedSourceBranch: string; selectedTargetBranch: string }) => (
...
vi.mock('../../constants', () => ({
  JOB_TYPE_CONFIG: {
    MAINTENANCE: { label: 'Maintenance', icon: () => null, color: 'text-orange-500' },
    CODE_REVIEW: { label: 'Code Review', icon: () => null, color: 'text-blue-500' },
    TEST_COVERAGE: { label: 'Test Coverage', icon: () => null, color: 'text-green-500' },
    SECURITY_AUDIT: { label: 'Security Audit', icon: () => null, color: 'text-red-500' },
    PERFORMANCE: { label: 'Performance', icon: () => null, color: 'text-yellow-500' },
    DOCUMENTATION: { label: 'Documentation', icon: () => null, color: 'text-purple-500' },
...
describe('WorkspaceSection', () => {
  ...
  it('passes LOCAL workspace type when set', () => {
    ...
    expect(screen.getByTestId('workspace-selector')).toHaveTextContent('type=LOCAL')
  })
})
...
describe('ModelSection', () => {
  it('renders ModelSelector with selected model', () => {
    ...
    expect(screen.getByTestId('model-selector')).toHaveTextContent('model=gpt-4o')
  })
})
