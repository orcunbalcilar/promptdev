...
${stepList}`);
  ...
  // Git workflow instructions for version-controlled tasks
...
3. Commit with a descriptive message: \`git commit -m "${task.commitMessagePattern ? task.commitMessagePattern.replace("{message}", "<describe changes>") : `[${task.jiraIssueKey || "promptdev"}] <describe changes>`}"\`
...
async function handleSessionIdle(
  ...
  if (task.iterative) {
    ...
    if (currentIteration < maxIterations) {
      const completionMet = await checkCompletionCriteria(task, lastMessage);
