...
describe("getMediaCategory", () => {
  it("returns correct category for MIME types", () => {
    expect(
      getMediaCategory(makeFileAttachment({ mediaType: "image/png" })),
    ).toBe("image");
    expect(
      getMediaCategory(makeFileAttachment({ mediaType: "video/mp4" })),
    ).toBe("video");
    expect(
      getMediaCategory(makeFileAttachment({ mediaType: "audio/mpeg" })),
    ).toBe("audio");
    expect(
      getMediaCategory(makeFileAttachment({ mediaType: "application/pdf" })),
    ).toBe("document");
    expect(
      getMediaCategory(makeFileAttachment({ mediaType: "text/plain" })),
    ).toBe("document");
    expect(getMediaCategory(makeFileAttachment({ mediaType: "" }))).toBe(
      "unknown",
    );
...
describe("getAttachmentLabel", () => {
  it("returns correct label", () => {
    ...
    expect(
      getAttachmentLabel(makeFileAttachment({ filename: undefined })),
    ).toBe("Image");
    expect(
      getAttachmentLabel(
        makeFileAttachment({
    ...
    expect(
      getAttachmentLabel(
        makeSourceAttachment({ title: undefined, filename: "doc.pdf" }),
      ),
    ).toBe("doc.pdf");
    expect(
      getAttachmentLabel(
        makeSourceAttachment({ title: undefined, filename: undefined }),
      ),
    ).toBe("Source");
  });
});
...
describe("AttachmentPreview – video branch", () => {
  ...
  it("renders icon for video without URL", () => {
    const { container } = render(
      <Attachments>
        <Attachment
          data={makeFileAttachment({ mediaType: "video/mp4", url: undefined })}
        >
          <AttachmentPreview />
        </Attachment>
      </Attachments>,
  ...
  it("renders icon for audio type", () => {
    const { container } = render(
      <Attachments>
        <Attachment
          data={makeFileAttachment({ mediaType: "audio/mpeg", url: undefined })}
        >
          <AttachmentPreview />
        </Attachment>
      </Attachments>,
  ...
  it("renders fallbackIcon when provided", () => {
    render(
      <Attachments>
        <Attachment
          data={makeFileAttachment({ mediaType: "text/plain", url: undefined })}
        >
          <AttachmentPreview
...
describe("getMediaCategory – edge cases", () => {
  it("returns unknown for undefined mediaType", () => {
    expect(getMediaCategory(makeFileAttachment({ mediaType: undefined }))).toBe(
      "unknown",
    );
  });
  ...
  it("returns document for text/* types", () => {
    expect(
      getMediaCategory(makeFileAttachment({ mediaType: "text/html" })),
    ).toBe("document");
  });

  it("returns unknown for custom MIME type", () => {
    expect(
      getMediaCategory(makeFileAttachment({ mediaType: "model/gltf-binary" })),
    ).toBe("unknown");
  });
});
