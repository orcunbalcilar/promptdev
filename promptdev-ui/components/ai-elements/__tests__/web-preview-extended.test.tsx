...
vi.mock('@/components/ui/tooltip', () => ({
  ...
  TooltipTrigger: React.forwardRef(({ children, ...props }: React.PropsWithChildren<Record<string, unknown>>, ref: React.Ref<HTMLElement>) => <span ref={ref as React.Ref<HTMLSpanElement>} {...props}>{children}</span>),
...
describe('WebPreviewUrl – URL navigation', () => {
  ...
  it('handles controlled value prop', () => {
    ...
    expect(screen.getByDisplayValue('https://controlled.com')).toBeInTheDocument()
  })
...
describe('WebPreviewBody', () => {
  ...
  it('renders loading slot when provided', () => {
    render(
      <WebPreview defaultUrl="https://example.com">
        <WebPreviewBody loading={<div data-testid="loading-spinner">Loading...</div>} />
      </WebPreview>
    )
...
describe('WebPreviewConsole', () => {
  ...
  it('shows log messages when expanded', async () => {
    const user = userEvent.setup()
    const logs = [
      { level: 'log' as const, message: 'App started', timestamp: new Date('2026-01-01T10:00:00') },
      { level: 'error' as const, message: 'Failed to fetch data', timestamp: new Date('2026-01-01T10:00:01') },
      { level: 'warn' as const, message: 'Deprecated API usage', timestamp: new Date('2026-01-01T10:00:02') },
    ]
  ...
  it('renders timestamp with each log entry', async () => {
    const user = userEvent.setup()
    const logs = [
      { level: 'log' as const, message: 'Test message', timestamp: new Date('2026-06-15T14:30:45') },
    ]
...
describe('WebPreviewNavigationButton', () => {
  ...
  it('applies additional props', () => {
    render(
      <WebPreview>
        <WebPreviewNavigation>
          <WebPreviewNavigationButton tooltip="Refresh" data-testid="refresh-btn">
            ↻
          </WebPreviewNavigationButton>
        </WebPreviewNavigation>
      </WebPreview>
