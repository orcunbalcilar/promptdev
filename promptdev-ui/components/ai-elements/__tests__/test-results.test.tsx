...
describe("TestSuiteStats", () => {
  it("renders counts", () => {
    render(<TestSuiteStats passed={5} failed={2} skipped={1} />);
  ...
  it("only shows non-zero stats", () => {
    render(<TestSuiteStats passed={3} failed={0} skipped={0} />);
...
describe("Test", () => {
  ...
  it("renders default layout with name, status and duration", () => {
    render(<Test name="should pass" status="passed" duration={42} />);
...
describe("TestStatus", () => {
  it.each([["passed"], ["failed"], ["skipped"], ["running"]] as const)(
    "shows correct icon for %s status",
    (status) => {
      const { container } = render(
...
describe("TestErrorMessage", () => {
  it("renders message", () => {
    render(<TestErrorMessage>Expected true to be false</TestErrorMessage>);
