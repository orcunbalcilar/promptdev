...
vi.mock('@xyflow/react', () => ({
  ...
  Controls: ({ className, ...props }: any) => <div data-testid="controls" className={className} {...props} />,
  Handle: ({ type, position, ...props }: any) => <div data-testid={`handle-${type}`} data-position={position} {...props} />,
  Position: { Top: 'top', Bottom: 'bottom', Left: 'left', Right: 'right' },
  NodeToolbar: ({ children, ...props }: any) => <div data-testid="node-toolbar" {...props}>{children}</div>,
  Panel: ({ children, className, ...props }: any) => <div data-testid="panel" className={className} {...props}>{children}</div>,
  BaseEdge: ({ id, path, style, ...props }: any) => <path data-testid={`edge-${id}`} d={path} style={style} {...props} />,
  useInternalNode: vi.fn().mockReturnValue({
    internals: {
      handleBounds: {
        source: [{ x: 0, y: 0, width: 10, height: 10, position: 'right', id: null }],
        target: [{ x: 0, y: 0, width: 10, height: 10, position: 'left', id: null }],
      },
