...
describe("PromptInput – extended", () => {
  ...
  it('triggers onError with "max_file_size" when file is too large', async () => {
    ...
    const bigFile = new File(["a".repeat(100)], "big.png", {
      type: "image/png",
    });
    ...
    render(
      <Wrapper>
        <PromptInput onSubmit={vi.fn()}>
          <PromptInputTools>
            <PromptInputButton
              tooltip={{ content: "Format", shortcut: "⌘B", side: "bottom" }}
            >
              B
            </PromptInputButton>
          </PromptInputTools>
        ...
        <PromptInput onSubmit={vi.fn()}>
          <PromptInputTools>
            <PromptInputSelect>
              ...
              <PromptInputSelectContent>
                <PromptInputSelectItem value="opt1">
                  Option 1
                </PromptInputSelectItem>
              </PromptInputSelectContent>
            </PromptInputSelect>
    ...
    function ProviderConsumer() {
      ...
      return (
        <div>
          ...
          <button data-testid="clear" onClick={() => ctrl.attachments.clear()}>
            clear
          </button>
        </div>
      );
  ...
  it("converts blob URL to data URL via FileReader on submit", async () => {
    ...
    globalThis.FileReader = vi.fn(function (this: {
      result: string | null;
      onloadend: (() => void) | null;
      onerror: (() => void) | null;
      readAsDataURL: () => void;
  ...
  it("handles global drag-and-drop when globalDrop is true", async () => {
    ...
    const droppedFile = new File(["global"], "global.png", {
      type: "image/png",
    });
    const files = [droppedFile] as unknown as FileList;
    await act(() => {
  ...
  it("clears input and files after async onSubmit resolves with provider", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByDisplayValue("async clear me"),
  ...
  it("clears provider text after sync onSubmit succeeds", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByDisplayValue("sync clear me"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("provider addWithProviderValidation caps files and still adds partial", async () => {
    ...
    function ProviderCounter() {
      const ctrl = usePromptInputController();
      return (
        <span data-testid="p-count2">{ctrl.attachments.files.length}</span>
      );
    }
  ...
  it("removeLocal revokes blob URL of removed file", async () => {
    ...
    fireEvent.change(fileInput, {
      target: { files: [new File(["x"], "rm.png", { type: "image/png" })] },
    });

    await waitFor(() =>
      expect(screen.getByTestId("lcount").textContent).toBe("1"),
    );
    (URL.revokeObjectURL as ReturnType<typeof vi.fn>).mockClear();
    await userEvent.click(screen.getByTestId("lrm-rm.png"));

    await waitFor(() =>
      expect(screen.getByTestId("lcount").textContent).toBe("0"),
    );
    expect(URL.revokeObjectURL).toHaveBeenCalled();
  });
  ...
  it("local clear revokes all blob URLs", async () => {
    function LocalClear() {
      ...
      return (
        <div>
          ...
          <button data-testid="lclear" onClick={attachments.clear}>
            clear
          </button>
        </div>
      );
    ...
    fireEvent.change(fileInput, {
      target: {
        files: [
          new File(["a"], "c1.png", { type: "image/png" }),
          new File(["b"], "c2.png", { type: "image/png" }),
        ],
      },
    });

    await waitFor(() =>
      expect(screen.getByTestId("lccount").textContent).toBe("2"),
    );
    (URL.revokeObjectURL as ReturnType<typeof vi.fn>).mockClear();
    await userEvent.click(screen.getByTestId("lclear"));

    await waitFor(() =>
      expect(screen.getByTestId("lccount").textContent).toBe("0"),
    );
  ...
  it("drag events without files type are no-op", () => {
    ...
    Object.defineProperty(drop2, "dataTransfer", {
      value: { types: ["text/plain"], files: null },
    });
  ...
  it("renders button with object tooltip without side (defaults to top)", () => {
    render(
      <Wrapper>
        <PromptInput onSubmit={vi.fn()}>
          <PromptInputTools>
            <PromptInputButton tooltip={{ content: "No Side" }}>
              NS
            </PromptInputButton>
          </PromptInputTools>
        </PromptInput>
  ...
  it("provider adds valid subset when some files exceed maxFileSize", async () => {
    ...
    function PVCounter() {
      const ctrl = usePromptInputController();
      return (
        <span data-testid="pv-count">{ctrl.attachments.files.length}</span>
      );
    }
