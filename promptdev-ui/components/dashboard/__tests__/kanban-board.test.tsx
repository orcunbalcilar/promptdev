...
describe("KanbanBoard", () => {
  ...
  it("shows task count in each column header", () => {
    ...
    const completedColumn = screen
      ...
      .closest(".flex.flex-col")!;
    expect(completedColumn.querySelector(".count-badge")).toHaveTextContent(
      "2",
    );
    ...
    const failedColumn = screen
      ...
      .closest(".flex.flex-col")!;
    expect(failedColumn.querySelector(".count-badge")).toHaveTextContent("1");
  });
  ...
  it("calls onTaskClick when a task card is clicked", async () => {
    ...
    renderWithProviders(
      <KanbanBoard tasks={[task]} onTaskClick={onTaskClick} />,
    );
