...
describe("TaskCard", () => {
  ...
  it("renders task prompt (truncated)", () => {
    const task = createTask({
      prompt: "Implement a feature that does something very specific",
    });
    renderWithProviders(<TaskCard task={task} />);

    expect(
      screen.getByText("Implement a feature that does something very specific"),
    ).toBeInTheDocument();
  });
  ...
  it("shows error preview when task status is FAILED and errorMessage is set", () => {
    ...
    expect(
      screen.getByText("NullPointerException at line 42"),
    ).toBeInTheDocument();
  });
