...
vi.mock('@/lib/sse-client', () => ({
  createSseSubscription: vi.fn((opts: { onMessage: (e: MessageEvent) => void }) => {
...
describe('DashboardView – SSE events', () => {
  it('shows success toast when task is COMPLETED', async () => {
    const tasks = [createTask({ id: '1', status: 'IN_PROGRESS', title: 'Build App' })]
    ...
    capturedSseCallback!(new MessageEvent('task-update', {
      data: JSON.stringify({
        id: '1',
        status: 'COMPLETED',
        title: 'Build App',
  ...
  it('shows error toast when task FAILED', async () => {
    const tasks = [createTask({ id: '1', status: 'IN_PROGRESS', title: 'Deploy' })]
    const data = createPagedResponse(tasks)
    renderWithProviders(<DashboardView initialTasks={data} />)
  ...
  it('cleans up SSE subscription on unmount', () => {
    ...
    const data = createPagedResponse(tasks)
    const { unmount } = renderWithProviders(<DashboardView initialTasks={data} />)
...
describe('DashboardView – navigation', () => {
  ...
  it('SSE toast action navigates to task', async () => {
    const tasks = [createTask({ id: 'nav-1', status: 'IN_PROGRESS', title: 'Nav Task' })]
    const data = createPagedResponse(tasks)
    renderWithProviders(<DashboardView initialTasks={data} />)
