...
describe("JobCard - extended", () => {
  ...
  it("calls deleteScheduledJob when confirm is accepted", async () => {
    ...
    expect(globalThis.confirm).toHaveBeenCalledWith(
      'Delete scheduled job "Nightly Tests"?',
    );
  ...
  it("does not render description when absent", () => {
    renderWithProviders(<JobCard job={makeJob({ description: undefined })} />);
  ...
  it("uses CUSTOM config for unknown job type", () => {
    renderWithProviders(
      <JobCard
        job={makeJob({ jobType: "UNKNOWN_TYPE" as ScheduledJob["jobType"] })}
      />,
    );
  ...
  it("does not render nextRunAt when absent", () => {
    renderWithProviders(<JobCard job={makeJob({ nextRunAt: undefined })} />);
    expect(screen.queryByText(/next run/i)).not.toBeInTheDocument();
  });

  it("does not render lastRunAt when absent", () => {
    renderWithProviders(<JobCard job={makeJob({ lastRunAt: undefined })} />);
