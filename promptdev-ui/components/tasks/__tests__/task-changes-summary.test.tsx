...
vi.mock("@/components/ai-elements/file-tree", () => ({
  FileTree: ({ children }: MockChildrenProps) => (
    <div data-testid="file-tree">{children}</div>
  ),
  FileTreeFile: ({ name }: MockNameProps) => (
...
vi.mock("@/components/ai-elements/code-block", () => ({
  CodeBlock: ({ children }: MockChildrenProps) => (
  ...
  CodeBlockFilename: ({ children }: MockChildrenProps) => (
    <span>{children}</span>
  ),
...
vi.mock("@/components/ai-elements/commit", () => ({
  ...
  CommitFileStatus: ({ status }: Readonly<{ status: string }>) => (
    <span data-testid="commit-file-status">{status}</span>
  ),
  CommitFileIcon: () => <span />,
  CommitFilePath: ({ children }: MockChildrenProps) => (
...
vi.mock("@/components/ai-elements/package-info", () => ({
  PackageInfo: ({
    ...
    newVersion,
    changeType,
  }: Readonly<{ name: string; newVersion?: string; changeType: string }>) => (
...
vi.mock("@/components/ai-elements/test-results", () => ({
  ...
  TestResultsContent: ({ children }: MockChildrenProps) => (
    <div>{children}</div>
  ),
  TestSuite: ({ children }: MockChildrenProps) => (
    <div data-testid="test-suite">{children}</div>
  ),
  TestSuiteName: () => <span />,
