...
const mockCreateTask = vi
  .fn()
  .mockResolvedValue({ id: "task-1", title: "Test", status: "PENDING" });
const mockStartTask = vi
  .fn()
  .mockResolvedValue({ id: "task-1", status: "QUEUED" });

vi.mock("@/lib/api", () => ({
  getProjects: vi.fn().mockResolvedValue([
    { key: "PRJ1", name: "Project One", description: "First project" },
    { key: "PRJ2", name: "Project Two", description: "Second project" },
  ]),
...
describe("CreateTaskDialog", () => {
  ...
  it("should auto-start the task after creation (regular task behavior)", async () => {
    // Regular tasks (non-Jira) auto-start: createTask is called first,
    // then startTask is called with the returned task ID
    const task = await mockCreateTask({
      title: "Test",
      prompt: "test",
  ...
  describe("Multi-project support", () => {
    it("should show project selector in Bitbucket mode", async () => {
      const user = userEvent.setup();
      renderWithProviders(<CreateTaskDialog />);
    ...
    it("should show project selector before repository selector", async () => {
      const user = userEvent.setup();
      renderWithProviders(<CreateTaskDialog />);
    ...
    it("should show project and repository selectors in Bitbucket mode", async () => {
      const user = userEvent.setup();
      renderWithProviders(<CreateTaskDialog />);
  ...
  describe("New features: review, jira, skills", () => {
    ...
    it("should hide review model selector when review is disabled", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText("Review Model (optional)"),
