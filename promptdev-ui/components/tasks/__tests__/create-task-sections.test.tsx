...
vi.mock('@/components/shared/workspace-selector', () => ({
  WorkspaceSelector: (props: Record<string, unknown>) => (
    <div data-testid="workspace-selector">Workspace: {props.workspaceType as string}</div>
  ),
}))

vi.mock('@/components/shared/branch-selector', () => ({
  BranchSelector: () => <div data-testid="branch-selector">Branch Selector</div>,
}))
...
vi.mock('@/components/ai-elements/suggestion', () => ({
  ...
  Suggestion: ({ suggestion }: { suggestion: string }) => <button data-testid="suggestion">{suggestion}</button>,
}))
