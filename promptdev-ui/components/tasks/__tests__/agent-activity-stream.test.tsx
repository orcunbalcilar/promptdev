...
vi.mock("@/components/ai-elements/reasoning", () => ({
  Reasoning: ({ children }: WithChildren) => (
    <div data-testid="reasoning">{children}</div>
  ),
  ReasoningTrigger: () => (
    <button data-testid="reasoning-trigger">Thinking</button>
  ),
...
vi.mock("@/components/ai-elements/tool", () => ({
  ...
  ToolContent: ({ children }: WithChildren) => (
...
vi.mock("@/components/ai-elements/commit", () => ({
  Commit: ({ children }: WithChildren) => (
    <div data-testid="commit">{children}</div>
  ),
  CommitHeader: ({ children }: WithChildren) => (
    <div data-testid="commit-header">{children}</div>
  ),
  CommitHash: ({ children }: WithChildren) => (
    <span data-testid="commit-hash">{children}</span>
  ),
  CommitMessage: ({ children }: WithChildren) => (
    <span data-testid="commit-message">{children}</span>
  ),
  CommitInfo: ({ children }: WithChildren) => (
    <div data-testid="commit-info">{children}</div>
  ),
  CommitContent: ({ children }: WithChildren) => (
    <div data-testid="commit-content">{children}</div>
  ),
  CommitFiles: ({ children }: WithChildren) => (
    <div data-testid="commit-files">{children}</div>
  ),
  CommitFile: ({ children }: WithChildren) => (
    <div data-testid="commit-file">{children}</div>
  ),
...
vi.mock("@/components/ai-elements/stack-trace", () => ({
  ...
  StackTraceErrorMessage: () => (
    <span data-testid="stack-trace-error-message" />
  ),
  ...
  StackTraceExpandButton: () => (
    <button data-testid="stack-trace-expand" aria-label="Expand stack trace" />
  ),
}));
...
vi.mock("@/components/ai-elements/shimmer", () => ({
  Shimmer: ({ children }: WithChildren) => (
    <div data-testid="shimmer">{children}</div>
  ),
}));

vi.mock("@/components/ai-elements/queue", () => ({
  Queue: ({ children }: WithChildren) => (
    <div data-testid="queue">{children}</div>
  ),
  QueueItem: ({ children }: WithChildren) => (
...
describe("AgentActivityStream", () => {
  ...
  it('renders empty state with "Waiting for agent activity" when no events', () => {
    render(
      <AgentActivityStream events={[]} task={defaultTask} isLive={true} />,
    );
    expect(
  ...
  it("renders TASK_CREATED event as queued item", () => {
    ...
    render(
      <AgentActivityStream events={events} task={defaultTask} isLive={false} />,
    );
  ...
  it("renders GIT_COMMIT event with commit component", () => {
    ...
    expect(screen.getByTestId("commit-file-path")).toHaveTextContent(
      "src/index.ts",
    );
  });
  ...
  it("renders TESTS_PASSED with test results component", () => {
    ...
    expect(screen.getByTestId("test-case")).toHaveTextContent(
      "should work: passed",
    );
  });
  ...
  it("does not show shimmer when task is COMPLETED", () => {
    const task = createTask({ status: "COMPLETED" });
    const events = [
      createEvent({ eventType: "TASK_COMPLETED", message: "Done" }),
    ];
    render(<AgentActivityStream events={events} task={task} isLive={true} />);
