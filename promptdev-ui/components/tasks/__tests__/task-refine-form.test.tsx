...
describe('TaskRefineForm', () => {
  it('should show refinement banner for pending Jira task', () => {
    ...
    expect(screen.getByText('This Jira task is awaiting refinement')).toBeInTheDocument()
  ...
  it('should populate form fields with task data', async () => {
    ...
    expect((promptInput as HTMLTextAreaElement).value).toContain('Jira Issue: PROJ-123')
  })
  ...
  it('should call updateTask when Save is clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateTask).toHaveBeenCalledWith('task-jira-1', expect.objectContaining({
  ...
  it('should close editing form when Cancel is clicked', async () => {
    ...
    expect(screen.queryByText('Refine Task Before Starting')).not.toBeInTheDocument()
    expect(screen.getByText('This Jira task is awaiting refinement')).toBeInTheDocument()
  })
})
