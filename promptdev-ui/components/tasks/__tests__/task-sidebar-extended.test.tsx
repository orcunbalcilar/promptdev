...
vi.mock('@/components/ai-elements/progress-bar', () => ({
  ...
  ProgressBarValue: ({ children }: { children?: ReactNode }) => <span data-testid="progress-value">{children}</span>,
...
const baseProps = {
  ...
  isLive: false,
  isProcessing: false,
  models: [{ id: 'gpt-5.2', name: 'GPT-5.2', vendor: 'openai' }] as Array<{ id: string; name: string; vendor: string }>,
  allEvents: [],
}
...
describe('TaskSidebar – extended', () => {
  ...
  it('renders pull request link when present', () => {
    ...
    expect(link.closest('a')).toHaveAttribute('href', 'https://bitbucket.org/pr/1')
  ...
  it('renders resume count with singular form', () => {
    render(
      <TaskSidebar
        {...baseProps}
        task={{ ...baseTask, resumeCount: 1 }}
      />,
    )
    expect(screen.getByText((text) => text.includes('Resumed 1 time') && !text.includes('times'))).toBeInTheDocument()
  })
  ...
  it('renders prompt text', () => {
    ...
    expect(screen.getByText('Implement a secure login page')).toBeInTheDocument()
  })
  ...
  it('renders iterative session card when task is iterative', () => {
    render(
      <TaskSidebar
        {...baseProps}
        task={{ ...baseTask, iterative: true, currentIteration: 3, maxIterations: 10 }}
      />,
    )
  ...
  it('uses defaults for iterative card when values are null', () => {
    render(
      <TaskSidebar
        {...baseProps}
        task={{ ...baseTask, iterative: true, currentIteration: null, maxIterations: null }}
      />,
    )
