...
describe("IterativeSection", () => {
  ...
  it("renders description text", () => {
    render(<IterativeSection />);
    expect(
      screen.getByText(/agent iterates until all steps complete/i),
    ).toBeInTheDocument();
  });
});
...
describe("ReviewSection", () => {
  ...
  it('shows "Same as task model" option and model list', async () => {
    ...
    expect(
      screen.getAllByText("Same as task model").length,
    ).toBeGreaterThanOrEqual(1);
    expect(screen.getByRole("option", { name: "GPT-5.2" })).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: "Claude 4" }),
    ).toBeInTheDocument();
  });
  ...
  it("renders description text", () => {
    render(<ReviewSection />);
    expect(
      screen.getByText(/automatically review generated code/i),
    ).toBeInTheDocument();
  });
