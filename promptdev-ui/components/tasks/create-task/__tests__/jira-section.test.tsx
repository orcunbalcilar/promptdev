...
describe("JiraSection", () => {
  ...
  it("fetches and shows triage panel on success", async () => {
    ...
    expect(
      screen.getByText("Users cannot sign in with SSO"),
    ).toBeInTheDocument();
  ...
  it("pre-fills title and prompt from Jira issue", async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockSetPrompt).toHaveBeenCalledWith(
        expect.stringContaining("Jira Issue: PROJ-123"),
      );
    });
  });
  ...
  it("shows generic error message for non-Error throws", async () => {
    ...
    expect(
      await screen.findByText("Failed to fetch Jira issue"),
    ).toBeInTheDocument();
  });
  ...
  it("renders issue without description", async () => {
    ...
    expect(
      await screen.findByText(/no description provided in jira/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows suggestion for missing description", async () => {
    ...
    expect(
      await screen.findByText(/add a detailed description/i),
    ).toBeInTheDocument();
  });
  ...
  it("does not show empty description suggestion when description exists", async () => {
    ...
    expect(
      screen.queryByText(/add a detailed description/i),
  ...
  it("renders triage suggestions section", async () => {
    ...
    expect(
      await screen.findByText(/suggestions for a better prompt/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/add specific acceptance criteria/i),
    ).toBeInTheDocument();
  });
