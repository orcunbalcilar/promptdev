...
describe('BitbucketCard', () => {
  ...
  it('pre-fills values from profile', () => {
    ...
    expect(screen.getByLabelText('Bitbucket Server URL')).toHaveValue('https://bb.example.com')
  ...
  it('calls updateUserSettings on save', async () => {
    ...
    await user.type(screen.getByLabelText('Bitbucket Server URL'), 'https://bb.co')
  ...
  it('shows save button', () => {
    renderWithProviders(<BitbucketCard userId="u1" profile={makeProfile()} />)
    expect(screen.getByRole('button', { name: /save bitbucket/i })).toBeInTheDocument()
  })
})
