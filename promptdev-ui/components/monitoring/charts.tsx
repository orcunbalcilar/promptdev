  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Operations over the selected time period
        </CardDescription>
      </CardHeader>
      <CardContent>
...
export function OperationsByTypeChart({
  data,
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Tool execution count and average duration
        </CardDescription>
      </CardHeader>
      <CardContent>
...
export function SessionsByModelChart({
  data,
}: Readonly<{ data: Record<string, number> }>) {
  const entries = Object.entries(data).sort((a, b) => b[1] - a[1]);
  if (!entries.length) return null;
