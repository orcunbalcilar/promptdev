...
vi.mock('@/components/ai-elements/tool', () => ({
  ...
  ToolInput: ({ input }: { input?: unknown }) => <div data-testid="tool-input">{JSON.stringify(input)}</div>,
...
vi.mock('@/components/ai-elements/stack-trace', () => ({
  StackTrace: ({ children }: { children: React.ReactNode }) => <div data-testid="stack-trace">{children}</div>,
...
vi.mock('@/components/ai-elements/progress-bar', () => ({
  ProgressBar: ({ children }: { children: React.ReactNode }) => <div data-testid="progress-bar">{children}</div>,
  ProgressBarLabel: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
  ProgressBarValue: ({ children }: { children: React.ReactNode }) => <span>{children}</span>,
...
describe('SessionDetail', () => {
  it('renders session info', () => {
    ...
    renderWithProviders(
      <SessionDetail session={session} onBack={vi.fn()} />,
    )
  ...
  it('shows status indicator for ACTIVE session', () => {
    ...
    expect(screen.getByTestId('status-indicator')).toHaveTextContent('streaming')
  })
  ...
  it('shows empty state when no operations', async () => {
    ...
    expect(await screen.findByText(/no operations recorded/i)).toBeInTheDocument()
  })
