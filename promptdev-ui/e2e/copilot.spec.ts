...
test.describe('Copilot Page', () => {
  ...
  test('shows start session dialog by default', async ({ page }) => {
    ...
    const hasStartDialog = await page.getByText(/Start|New Session|Select.*Model/i).first().isVisible().catch(() => false);
  ...
  test('displays model selection in start dialog', async ({ page }) => {
    ...
    const hasModelSelect = await page.getByText(/model/i).first().isVisible().catch(() => false);
    const hasStartButton = await page.getByRole('button', { name: /Start/i }).first().isVisible().catch(() => false);
  ...
  test('shows sidebar toggle button', async ({ page }) => {
    ...
    const sidebarToggle = page.getByRole('button', { name: /sidebar|panel|history/i });
    const hasSidebarToggle = await sidebarToggle.first().isVisible().catch(() => false);

    // The sidebar itself should show session history heading
    const hasSidebar = await page.getByText(/Session History|History/i).first().isVisible().catch(() => false);
