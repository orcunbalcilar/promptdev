...
test.describe('Home Page', () => {
  ...
  test('renders dashboard content or error state without crashing', async ({ page }) => {
    ...
    const hasKanban = await page.locator('.kanban-column').first().isVisible().catch(() => false);
    const hasEmptyState = await page.getByText('No tasks yet').isVisible().catch(() => false);
    const hasError = await page.getByText('Failed to load tasks').isVisible().catch(() => false);
