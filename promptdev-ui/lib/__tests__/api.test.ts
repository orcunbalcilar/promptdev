...
describe("API Client", () => {
  ...
  describe("getTaskEvents", () => {
    it("should GET /tasks/:id/events", async () => {
      const events = [
        { id: "e1", eventType: "TASK_CREATED", message: "Created" },
      ];
  ...
  describe("retryTask", () => {
    it("should POST /tasks/:id/retry", async () => {
      mockFetch.mockResolvedValue(
        jsonResponse({ id: "task-1", status: "PENDING" }),
      );
  ...
  describe("startTask", () => {
    it("should POST /tasks/:id/start", async () => {
      mockFetch.mockResolvedValue(
        jsonResponse({ id: "task-1", status: "IN_PROGRESS" }),
      );
  ...
  describe("getBranches", () => {
    it("should GET /repositories/:slug/branches", async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toBe(
        `${API_BASE}/repositories/my-repo/branches`,
      );
      expect(result).toHaveLength(2);
  ...
  describe("getDefaultBranch", () => {
    it("should GET /repositories/:slug/default-branch", async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toBe(
        `${API_BASE}/repositories/my-repo/default-branch`,
      );
  ...
  describe("getScheduledJobs", () => {
    ...
    it("should GET /scheduled-jobs with type filter", async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toBe(
        `${API_BASE}/scheduled-jobs?type=SECURITY_AUDIT`,
      );
  ...
  describe("toggleScheduledJob", () => {
    it("should POST /scheduled-jobs/:id/toggle", async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toBe(
        `${API_BASE}/scheduled-jobs/job-1/toggle`,
      );
      expect(mockFetch.mock.calls[0][1].method).toBe("POST");
  ...
  describe("runScheduledJobNow", () => {
    it("should POST /scheduled-jobs/:id/run", async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toBe(
        `${API_BASE}/scheduled-jobs/job-1/run`,
      );
      expect(mockFetch.mock.calls[0][1].method).toBe("POST");
      expect(result).toEqual(task);
    });
  ...
  describe("getScheduledJobHistory", () => {
    it("should GET /scheduled-jobs/:id/history", async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toBe(
        `${API_BASE}/scheduled-jobs/job-1/history`,
      );
      expect(result).toHaveLength(2);
      ...
      mockFetch.mockImplementation(() => {
        ...
        if (callCount === 1) {
          return Promise.resolve(
            jsonResponse({ id: "task-exec", status: "QUEUED" }),
          );
        }
    ...
    it("should still return task when execute trigger fails", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "[API] Failed to trigger resumed task execution",
        ),
        expect.any(Error),
      );
      consoleSpy.mockRestore();
