...
describe('Jira API Client', () => {
  describe('searchJiraIssues', () => {
    it('should search issues by JQL query', async () => {
      const response = {
        issues: [
          { id: '1', key: 'PROJ-123', fields: { summary: 'Fix bug' } },
        ],
        startAt: 0,
        maxResults: 50,
        total: 1,
    ...
    it('should encode JQL in URL', async () => {
      ...
      const url = mockFetch.mock.calls[0][0] as string
      expect(url).toContain(encodeURIComponent('project = PROJ AND assignee = "john doe"'))
    })
  })
  ...
  describe('getJiraIssue', () => {
    ...
    it('should throw on 404', async () => {
      ...
      await expect(getJiraIssue('PROJ-999')).rejects.toThrow('Jira API request failed: 404')
    })
  })
  ...
  describe('getJiraTransitions', () => {
    it('should fetch available transitions', async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toContain('/jira/issues/PROJ-123/transitions')
  ...
  describe('getJiraProjectIssues', () => {
    it('should fetch issues by project key', async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toContain('/jira/projects/PROJ/issues')
  ...
  describe('error handling', () => {
    ...
    it('should throw on unauthorized', async () => {
      ...
      await expect(getJiraIssue('PROJ-1')).rejects.toThrow('Jira API request failed: 401')
    })
  })
})
