...
vi.mock("@/lib/copilot/client", () => {
  ...
  return {
    ...
    subscribeToSession: vi.fn(
      (sessionId: string, callback: (event: unknown) => void) => {
...
vi.mock("@/lib/services/bitbucket-service", () => ({
  getCloneUrl: vi
    .fn()
    .mockReturnValue("https://bitbucket.example.com/scm/proj/my-app.git"),
  getBitbucketConfig: vi
    .fn()
    .mockReturnValue({
...
describe("Task Orchestrator", () => {
  describe("executeTask", () => {
    ...
    it("should send AGENT_STARTED callback", async () => {
      ...
      const agentStartedCall = calls.find(
        ([callbackArg]: [{ eventType: string }]) =>
          callbackArg.eventType === "AGENT_STARTED",
      );
  ...
  describe("executeTask with BITBUCKET workspace", () => {
    ...
    it('should include "already cloned" in git workflow for BITBUCKET tasks', async () => {
      ...
      const systemMessage = (createCopilotSession as ReturnType<typeof vi.fn>)
        .mock.calls[0][0].systemMessage;
