...
describe("Copilot Client", () => {
  ...
  describe("ensureSqliteSupport", () => {
    ...
    it("should not duplicate --experimental-sqlite if already present", async () => {
      ...
      const count = (
        process.env.NODE_OPTIONS?.match(/--experimental-sqlite/g) || []
      ).length;
      expect(count).toBe(1);
    });
  });
  ...
  describe("sendMessage", () => {
    ...
    it("should throw if session not found", async () => {
      await expect(sendMessage("nonexistent", "test")).rejects.toThrow(
        "Session not found",
      );
    });
  ...
  describe("sendAndWait", () => {
    ...
    it("should throw if session not found", async () => {
      await expect(sendAndWait("nonexistent", "test")).rejects.toThrow(
        "Session not found",
      );
    });
  ...
  describe("getSessionMessages", () => {
    ...
    it("should throw for nonexistent session", async () => {
      await expect(getSessionMessages("nonexistent")).rejects.toThrow(
        "Session not found",
      );
    });
  });
