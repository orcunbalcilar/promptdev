...
export interface TaskTemplate {
  ...
  name: string;
  description: string;
  category: "feature" | "bugfix" | "refactor" | "testing" | "docs" | "security" | "custom";
  icon: string;
  prompt: string;
...
export const BUILT_IN_TEMPLATES: TaskTemplate[] = [
  {
    id: "feature-crud",
    name: "CRUD Feature",
    description: "Generate a complete CRUD feature with API routes, service layer, and UI components",
    category: "feature",
    icon: "🏗️",
    prompt: "Create a complete CRUD feature for {entity}. Include:\n- Database schema with Drizzle ORM\n- API routes (GET, POST, PUT, DELETE)\n- Service layer with validation\n- React components with forms\n- Unit tests for service and API",
  ...
  },
  {
    ...
    category: "security",
    icon: "🔒",
    prompt: "Perform a security audit on {scope}:\n1. Check for injection vulnerabilities\n2. Validate input sanitization\n3. Review authentication/authorization\n4. Check for sensitive data exposure\n5. Fix any issues found",
    defaults: { reviewEnabled: true },
    tags: ["security", "audit"],
  },
  {
...
export function interpolateTemplate(template: TaskTemplate, vars: Record<string, string>): string {
