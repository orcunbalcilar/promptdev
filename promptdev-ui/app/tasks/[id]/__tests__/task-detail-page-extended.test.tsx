...
const MOCK_EVENTS = [
  ...
  { id: 'e3', taskId: 'task-1', eventType: 'CODE_GENERATED', message: 'Code generated', timestamp: '2026-01-01T12:05:00Z' },
]
...
vi.mock('@/components/tasks/activity-stream/stream', () => ({
  AgentActivityStream: ({ events, isLive }: { events: unknown[]; isLive: boolean }) => (
    <div data-testid="activity-stream" data-live={isLive}>Events: {events.length}</div>
  ),
}))
...
vi.mock('@/components/tasks/task-header-actions', () => ({
  TaskHeaderActions: ({ task, showResumeForm, setShowResumeForm, onRetry, onCancel, onStart }: {
...
vi.mock('@/components/tasks/task-sidebar', () => ({
  TaskSidebar: ({ task, isLive, isProcessing, models, sessionDetails, allEvents }: {
...
vi.mock('@/components/tasks/task-refine-form', () => ({
  TaskRefineForm: ({ task, onStarted }: { task: { id: string }; onStarted: () => void }) => (
...
describe('TaskDetailPage – extended', () => {
  ...
  it('marks task as live when status is IN_PROGRESS', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('sidebar-processing')).toHaveTextContent('true')
    })
  })
  ...
  it('submits resume with prompt text and calls resumeTask', async () => {
    ...
    await user.type(screen.getByTestId('resume-input'), 'Please fix the auth module')
    await user.click(screen.getByText('Submit Resume'))

    await waitFor(() => {
      expect(mockResumeTask).toHaveBeenCalledWith('task-1', 'Please fix the auth module')
    })
  })
  ...
  it('shows refine form for PENDING task with jiraIssueKey', async () => {
    mockGetTask.mockResolvedValue({ ...MOCK_TASK, status: 'PENDING', jiraIssueKey: 'PROJ-42' })
    await renderPage()
  ...
  it('does not show refine form for PENDING task without jiraIssueKey', async () => {
    mockGetTask.mockResolvedValue({ ...MOCK_TASK, status: 'PENDING', jiraIssueKey: undefined })
    await renderPage()
  ...
  it('does not show refine form for non-PENDING task with jiraIssueKey', async () => {
    mockGetTask.mockResolvedValue({ ...MOCK_TASK, status: 'COMPLETED', jiraIssueKey: 'PROJ-42' })
    await renderPage()
  ...
  it('renders without copilotSessionId gracefully', async () => {
    mockGetTask.mockResolvedValue({ ...MOCK_TASK, copilotSessionId: undefined })
    await renderPage()
  ...
  it('passes events to changes summary tab', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('changes-summary')).toHaveTextContent('Changes: 3')
    })
  })
})
