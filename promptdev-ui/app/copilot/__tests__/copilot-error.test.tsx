...
describe("CopilotError", () => {
  ...
  it("should render the copilot error heading", () => {
    render(<CopilotError error={testError} reset={mockReset} />);
    expect(screen.getByText("Copilot session error")).toBeInTheDocument();
  });
  ...
  it("should log the error to console", () => {
    render(<CopilotError error={testError} reset={mockReset} />);
    expect(console.error).toHaveBeenCalledWith("Copilot error:", testError);
  });

  it("should display error with digest property", () => {
    const digestError = Object.assign(new Error("Copilot digest error"), {
      digest: "cop-123",
    });
    render(<CopilotError error={digestError} reset={mockReset} />);
    expect(screen.getByText("Copilot digest error")).toBeInTheDocument();
  });
});
