...
const mockModels = [
  ...
  { id: 'claude-sonnet-4.5', name: 'Claude Sonnet 4.5', description: 'Anthropic', provider: 'anthropic', billing: { multiplier: 1 }, capabilities: {} },
]
...
vi.mock('@/components/copilot/settings-dialog', () => ({
  SettingsDialog: ({ setModel, setReasoningEffort, models }: {
    ...
    <div data-testid="settings-dialog">
      ...
      <select data-testid="settings-model-select" title="Settings Model" onChange={(e) => setModel(e.target.value)}>
        {models.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
      </select>
      <select data-testid="settings-effort-select" title="Reasoning Effort" onChange={(e) => setReasoningEffort(e.target.value)}>
...
vi.mock('@/components/copilot/start-session-dialog', () => ({
  StartSessionDialog: ({ onStart, model, setModel, setReasoningEffort, models }: {
    onStart: () => void
    model: string
    setModel: (m: string) => void
    setReasoningEffort: (v: string) => void
    ...
    <div data-testid="start-dialog">
      ...
      <select data-testid="model-select" title="Model" value={model} onChange={(e) => setModel(e.target.value)}>
        {models.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
      </select>
      <select data-testid="effort-select" title="Effort" onChange={(e) => setReasoningEffort(e.target.value)}>
        <option value="low">Low</option>
        <option value="medium">Medium</option>
        <option value="high">High</option>
      </select>
...
describe('CopilotAgentPage – final coverage', () => {
  ...
  it('click "Find bugs" quick prompt sends correct message', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /start agent/i }))
    await waitFor(() => expect(screen.getByText('Find bugs')).toBeInTheDocument())
  ...
  it('renders session info footer when session active with messages', async () => {
    resetHookState({
      session: { id: 'abc12345-6789-xxxx-yyyy-zzzz', model: 'gpt-5.2', createdAt: '2026-01-01T12:00:00Z' },
      messages: [
  ...
  it('renders session footer with session model name', async () => {
    resetHookState({
      session: { id: 'sess-unique-1234', model: 'claude-sonnet-4.5', createdAt: '2026-02-15T10:30:00Z' },
  ...
  it('renders error state with "Failed to Initialize" heading and error message', async () => {
    ...
    await waitFor(() => {
      ...
      const errorMessages = screen.getAllByText('Connection refused: backend unavailable')
  ...
  it('error state Retry button calls createSession again', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /start agent/i }))
    await waitFor(() => expect(screen.getByText('Failed to Initialize')).toBeInTheDocument())
