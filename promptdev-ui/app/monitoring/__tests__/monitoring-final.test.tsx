...
function createWrapper() {
  ...
  function TestQueryProvider({ children }: Readonly<{ children: React.ReactNode }>) {
    return (
...
describe('MonitoringPage – final coverage', () => {
  ...
  it('activeTab value changes reflect correct tab content', async () => {
    ...
    expect(screen.getByRole('tab', { name: /sessions/i })).toHaveAttribute('data-state', 'active')
  ...
  it('shows session detail when clicking View button, and backs out', async () => {
    ...
    const dataRow = Array.from(tableRows).find(
      (row) => row.textContent?.includes('sdk-001')
    )
