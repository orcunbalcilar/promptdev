...
for (const [path, cov] of Object.entries(data)) {
  for (const t of targets) {
    ...
    if (path.includes("/ai-elements/" + t) && !path.includes("__tests__")) {
      ...
      const branchHit = Object.values(b).reduce(
      ...
      const uncovStmts = Object.keys(s)
        .filter((k) => s[k] === 0)
        .map((k) => sm[k].start.line);
      ...
      const fm = cov.fnMap;
      const uncovFns = Object.keys(f)
        .filter((k) => f[k] === 0)
        .map((k) => fm[k].loc.start.line);

      console.log(`\n${t}:`);
      console.log(
        `  Stmts: ${stmtHit}/${stmtTotal} (${((100 * stmtHit) / stmtTotal).toFixed(1)}%)`,
      );
      console.log(
        `  Branches: ${branchHit}/${branchTotal} (${((100 * branchHit) / branchTotal).toFixed(1)}%)`,
      );
      console.log(
        `  Functions: ${fnHit}/${fnTotal} (${((100 * fnHit) / fnTotal).toFixed(1)}%)`,
      );
      if (uncovStmts.length)
        console.log(
          `  Uncovered stmts: ${[...new Set(uncovStmts)].sort((a, b) => a - b).join(", ")}`,
        );
      if (uncovBranches.length)
        console.log(`  Uncovered branches: ${uncovBranches.join(", ")}`);
      if (uncovFns.length)
        console.log(`  Uncovered fns: ${uncovFns.join(", ")}`);
    }
  }
}
