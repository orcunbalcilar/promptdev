...
async function createAndConnect(result: { current: ReturnType<typeof useCopilotSession> }) {
  await act(async () => {
...
describe('useCopilotSession', () => {
  ...
  it('should initialize with idle state and no session', () => {
    const { result } = renderHook(() => useCopilotSession(), { wrapper: createWrapper() })
      ...
      if (url === '/api/copilot/models') {
        return Promise.resolve({
          ...
          json: () => Promise.resolve({
            models: [{ id: 'gpt-5-mini', name: 'GPT-5 Mini', capabilities: { supports: { reasoningEffort: false } } }],
          }),
        })
      }
      return Promise.resolve({ ok: true, json: () => Promise.resolve({}) })
  ...
  describe('handleEvent - assistant messages', () => {
    ...
    it('should handle assistant.message completing a turn', async () => {
      ...
      fireSSE('assistant.message', { content: 'Hello world!', messageId: 'msg-1' })
  ...
  describe('handleEvent - tool execution', () => {
    ...
    it('should handle tool.execution_complete', async () => {
      ...
      fireSSE('tool.execution_start', { toolName: 'read_file', toolId: 'tool-1', input: { path: '/tmp/file.ts' } })
      fireSSE('tool.execution_complete', { toolId: 'tool-1', output: 'file content...', duration: 120 })
    ...
    it('should handle tool.execution_end (legacy)', async () => {
      ...
      fireSSE('tool.execution_start', { toolName: 'write_file', toolId: 'tool-2', input: {} })
      fireSSE('tool.execution_end', { toolId: 'tool-2', error: 'Permission denied' })
  ...
  describe('handleEvent - error handling', () => {
    ...
    it('should handle session.error event', async () => {
      ...
      fireSSE('session.error', { message: 'Model unavailable', errorType: 'model_error' })
