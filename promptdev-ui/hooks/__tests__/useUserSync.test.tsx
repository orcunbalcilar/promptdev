...
describe("useUserSync", () => {
  ...
  it("should sync user after authentication", async () => {
    const mockSession = {
      user: {
    ...
    vi.mocked(userLib.syncUser).mockResolvedValue(mockProfile);
  ...
  it("should handle sync errors and not provide userId", async () => {
    ...
    const syncError = new Error("User sync failed");
  ...
  it("should fetch user profile directly if session ID is a UUID", async () => {
    ...
    vi.mocked(userLib.getUserProfile).mockResolvedValue(mockProfile);
