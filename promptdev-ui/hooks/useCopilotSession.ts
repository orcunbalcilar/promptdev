...
export function useCopilotSession(
  ...
  const connectToStream = useCallback(
    (sessionId: string) => {
      ...
      const eventSource = new EventSource(
        `/api/copilot/sessions/${sessionId}/stream`,
      );
  ...
  const createSession = useCallback(
    async (customOptions?: CreateSessionRequest) => {
      try {
        setError(null);
        setState("processing");
        setInputTokens(0);
        ...
        const supportsReasoning =
          targetModel?.capabilities.supports.reasoningEffort ?? false;
        /* v8 ignore stop */

        /* v8 ignore start — ternary + ?? branches for optional parameters */
        const sessionReasoningEffort = supportsReasoning
          ? (customOptions?.reasoningEffort ?? reasoningEffort)
          : undefined;
        const sessionSystemMessage =
        ...
        registerMonitoringSession({
          ...
          /* monitoring should not break the app */
        });
        /* v8 ignore stop */
    ...
    async (prompt: string) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/copilot/sessions/${session.id}/messages`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  const destroy = useCallback(async () => {
    ...
    try {
      ...
      endMonitoringSession(session.id).catch(() => {
        /* non-critical */
      });
      flushOperations().catch(() => {
        /* non-critical */
      });
            ...
            if (Array.isArray(msgData)) {
              const restored: CopilotMessage[] = msgData
                ...
                .map((e: Record<string, unknown>) => ({
                  ...
                  content: ((e.data as Record<string, unknown>)?.content ??
                    "") as string,
